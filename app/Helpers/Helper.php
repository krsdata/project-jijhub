...
//            }
    ...
    public function count_product( $id ) {
        ...
        //$result = \Zizhub\Admin\Models\ZizhubOrder::where( 'id','<' , 4 )->sum("LENGTH('product_id') - LENGTH(REPLACE('product_id', ',', '' ) ) +1");

        $sql = "SELECT sum((LENGTH( product_id ) - LENGTH( REPLACE( product_id, ',', '' ) ) +1 )) AS MyCol2Count
        FROM zizhuborders WHERE id <$id";        
        $records = DB::select($sql);        
        if(!empty($records[0]->MyCol2Count))
        {
            return $records[0]->MyCol2Count;
        }
        else
        {
            return false;
    ...
    public function zizpic_product_count($id)
    {
        $sql = "SELECT SUM( `zizpic_1_image` != '' ) as zizpic1, SUM( `zizpic_2_image` != '' ) as zizpic2 , SUM( `zizpic_3_image` != '' ) as zizpic3 FROM orders WHERE id <$id";        
        $records = DB::select($sql);        
        foreach($records as $rec)
        {           
            $order=(empty($rec->zizpic1)?0:$rec->zizpic1)+(empty($rec->zizpic2)?0:$rec->zizpic2)+(empty($rec->zizpic3)?0:$rec->zizpic3);
        }        
        if(!empty($order))
        {
            return $order;
