...
class ZizpicCopounCodesController extends Controller {
    ...
    public function store( ZizpicOrderRequest $request , ZizpicOrder $zizpicOrder ) {
        ...
        if ( $request->get( 'payment_method' ) == 'phone' ) {
            ...
            $user_name="Admin";                     
            $msg="Hi $user_name, <br>";            
                $msg.="A custmer purchase ".'Package-' . Input::get( 'zizhubpackage' )." <br> Total amount paid :". Input::get( 'total' );                               
                $body = $msg;               
            Mail::send('packages::zizhub.mail', array('content'=>$body),
    ...
    public function getPaymentStatus() {
        ...
        if ( isset( $paypalResponse[ 'ACK' ] ) && $paypalResponse[ 'ACK' ] == 'Success' ) {
            ...
            $orderObj->save();
            /********notification to admin*********/            
            $admin_email=$admin_email=Config::get( 'app.admin_email' );
            $user_name="Admin";                     
            $msg="Hi $user_name, <br>";            
            $msg="<br>A custmer purchase ".$params['name']." <br> Total amount paid :". $params['amount'];                               
            $body = $msg;
            $body;
            Mail::send('packages::zizhub.mail', array('content'=>$body),
                function($message) use ($admin_email,$user_name) {                   
